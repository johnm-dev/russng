...
main(int argc, char **argv) {
	...
	russ_forwarder_init(&(fwds[0]), STDIN_FILENO, conn->fds[0], -1, 65536, 0, 1);
	russ_forwarder_init(&(fwds[1]), conn->fds[1], STDOUT_FILENO, -1, 65536, 0, 1);
	russ_forwarder_init(&(fwds[2]), conn->fds[2], STDERR_FILENO, -1, 65536, 0, 1);
	conn->fds[0] = -1;
	conn->fds[1] = -1;
	conn->fds[2] = -1;
